...
class gameOb():
    def __init__(self, coordinates: Tuple, size: int, intensity, channel, reward, name: str):
...
class Qnetwork():
    def __init__(self, h_size):
        ...
        self.conv1 = tf.contrib.layers.convolution2d(
            inputs=self.imageIn, num_outputs=64, kernel_size=[8, 8], stride=[4, 4],
        ...
        self.Q = tf.reduce_mean(tf.multiply(self.Qout, self.actions_onehot), reduction_indices=1)
...
def updateTarget(op_holder, sess: tf.Session):
    for op in op_holder:
        sess.run(op)
